...
describe('computeImpact', () => {
  it('handles the material split: one article splits into many, citing rule is indirect', () => {
    ...
    expect(result.rules.direct.map((r) => r.ruleId)).toEqual(['RULE-ELIGIBILITY-01']);
    expect(result.rules.indirect.map((r) => r.ruleId)).toEqual(['RULE-SERVICE-02']);
    expect(result.rules.unaffected).toEqual([]);
    // ART-B is not re-listed in V2 and has no succession edge: reported, not guessed.
  ...
  it('handles many-to-one merges: all merged articles are direct change sources', () => {
    ...
    expect(result.missingSuccession).toEqual([]);
    // ART-W cites ART-Y which merges away; path must originate at ART-Y.
    expect(result.paths.find((p) => p.ruleId === 'RULE-W')?.nodes).toEqual(['ART-Y', 'ART-W']);
  });
  ...
  it('terminates on cross-reference cycles and reports each article once', () => {
    const graph: RevisionGraph = {
      ...
      references: [
        ...
        { versionId: 'V1', fromId: 'ART-C', toId: 'ART-A' }, // cycle: A cites B cites C cites A
      ],
    ...
    // Reverse edges: A<-B, B<-C, C<-A, so from changed A the walk is A->C->B.
    expect(result.paths.filter((p) => p.ruleId === 'RULE-C').map((p) => p.nodes)).toEqual([
      ['ART-A', 'ART-C'],
    ]);
    expect(result.paths.filter((p) => p.ruleId === 'RULE-B').map((p) => p.nodes)).toEqual([
      ['ART-A', 'ART-C', 'ART-B'],
    ]);
  });
  ...
  it('reports missing succession instead of guessing identity from identical labels', () => {
    const graph: RevisionGraph = {
      ...
      articles: [
        ...
        // Same label in V2, but NO succession edge: identity must not be inferred.
    ...
    // No change source at all: nothing is guessed into existence.
    expect(result.rules.direct).toEqual([]);
    expect(result.rules.indirect).toEqual([]);
    expect(result.rules.unaffected.map((r) => r.ruleId)).toEqual(['RULE-KEEP', 'RULE-OLD']);
  });
  ...
  it('keeps same-day versions with distinct statuses distinct in the query context', () => {
    ...
    // Draft target does not contain ART-A and declares no succession: missing, not guessed.
  ...
  it('is deterministic regardless of input ordering', () => {
    ...
    // Diamond-shaped references yield every simple path, deduplicated and
    // sorted lexicographically by node sequence.
    ...
    // N06 has no reference link to the changed set.
  ...
  it('scales on a larger layered graph with deduplicated, stably sorted paths', () => {
    ...
    // Exactly `width` distinct paths SRC -> L1-i -> SINK, no duplicates,
  ...
  it('throws UnknownVersionError for unknown versions', () => {
    expect(() => computeImpact(baseGraph(), { ...QUERY, fromVersion: 'NOPE' })).toThrow(
      UnknownVersionError,
    );
  });
});
